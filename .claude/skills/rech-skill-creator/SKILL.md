...
description: "Cria e valida pacotes de skill do Claude Code através de um pipeline com gates mecânicos e não-simulados: DISCOVER → CONTRACT → RISK CLASSIFICATION → DRAFT → SCHEMA VALIDATION → EVAL ISOLATION → SNAPSHOT/HASH → RUN EXECUTION → VALID_RUN_GATE → AGGREGATION → ARTIFACT_INTEGRITY_GATE → PACKAGE_RELEASE_GATE → PACKAGE. Nunca fabrica PASS: cada gate exige evidência real (manifesto de evals congelado/hasheado antes de qualquer run, hashes SHA-256 de todos os decision inputs, não só do candidato) e reporta UNVERIFIED/NOT_APPLICABLE/PARTIAL quando algo genuinamente não pode ser checado offline. Nunca inventa taxonomia paralela a RAF (uma tentativa anterior propôs 7 rótulos próprios e foi rejeitada por isso). Produz veredito RELEASABLE, BLOCKED ou INCONCLUSIVE sobre um pacote de skill específico — nunca decide release de produto RECH (isso é rech-release-gate), nunca audita conteúdo clínico de outra skill (isso é rech-deep-audit), nunca corrige bug de skill já existente e travada (isso é rech-fix). Use ao criar uma nova skill do zero, ao validar um pacote de skill antes de publicar, ou ao investigar por que uma skill não está sendo escolhida/roteada. NÃO dispare para alterar qualquer uma das 5 skills já travadas do RechHub (rech-deep-audit, rech-fix, rech-regression-guardian, rech-release-gate, rech-repo-context) nem para decidir se uma mudança em produto RECH pode mergear."
---
